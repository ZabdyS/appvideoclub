<script setup lang="ts">
  import { useGeneroStore } from '@/store/generos';
  ...
  const currentItem = reactive<Genero>({
    id : 0,
    descripcion: '',
    isActive: false
  });
  ...
  const items = computed(() => store.generos);
  ...
  const createItem = async () => {
    try {
      await store.createGenero(currentItem);
  ...
  const updateItem = async () => {
    try {
      await store.updateGenero(currentItem);
      toast.success("Operacion de Registro Completada correctamente");
    } catch (error: any) {
